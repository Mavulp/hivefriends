...
response=$(curl -v -XPUT --data '{"displayName": "Testing Name"}' "http://localhost:8080/api/settings/" -H "Content-Type: application/json" --oauth2-bearer $token)
printf "settings put response: %s\n\n" "$(printf "%s" "$response" | jq)"

response=$(curl -v "http://localhost:8080/api/settings/" -H "Content-Type: application/json" --oauth2-bearer $token)
printf "settings get response: %s\n\n" "$(printf "%s" "$response" | jq)"
