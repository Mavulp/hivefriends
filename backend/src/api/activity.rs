    ...
    state
        ...
        .call(move |conn| {
            ...
            let albums = album::get_all::get_albums(username, filters, &conn)?
                .into_iter()
                .map(|a| Activity::NewAlbum(a));

            let users = user::get_all(&conn)?
                .into_iter()
                .map(|u| Activity::NewUser(u));

            let comments = comment::get_all(&conn)?
                .into_iter()
                .map(|c| Activity::NewComment(c));

            let images = get_new_images(&conn)?
                .into_iter()
                .map(|i| Activity::NewImage(i));
...
impl PartialEq for Activity {
    fn eq(&self, other: &Activity) -> bool {
        ...
        let this_time = match self {
            NewAlbum(a) => a.published_at,
            NewComment(c) => c.created_at,
            NewUser(u) => u.created_at,
            NewImage(i) => i.published_at.unwrap(),
        };
...
mod test {
    ...
    async fn get_activity() {
        ...
        assert_matches!(result, Ok(Json(activities)) => {
            assert_matches!(&activities[0], Activity::NewImage(image) => {
                assert_eq!(expected_image, image.key);
            });

            assert_matches!(&activities[1], Activity::NewComment(comment) => {
                assert_eq!(expected_comment.text, comment.text);
            });

            assert_matches!(&activities[2], Activity::NewAlbum(album) => {
                assert_eq!(expected_album, album.key);
            });

            assert_matches!(&activities[3], Activity::NewUser(user) => {
